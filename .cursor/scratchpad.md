...
- [x] Executor: ONEDRIVE steps 1-4 — full implementation deployed DORMANT (24728e2): MicrosoftOneDriveProvider (+generic ConnectorTokenService/ConnectorOAuthException refactor, Google smoke re-verified 200), OneDriveImporter (Graph download + format=pdf conversion), onedrive/{start,callback,disconnect,browse,import-to-chat}.php, voice import-onedrive.php, Claara-native picker modal (onedrive-picker.js + .cfp-* styles), chat attach-menu 3rd option, Voice Studio OneDrive button, connectors.js mappings, MS_OAUTH_* placeholders in both .envs. UNBLOCKED 2026-07-09: Azure app registered from the Ebone tenant (personal MSAs cannot enter the Azure portal — AADSTS16000), MS_OAUTH_* creds copied to prod .env (backup .env.bak-ms), provider smoke-tested (auth URL OK) and connector_providers.onedrive flipped to is_enabled=1. Docs: docs/apis/onedrive_connector.md. Awaiting user browser QA (connect → chat import w/ docx conversion → voice import).
...
- [x] Executor: Connectors step 6 — chat composer Drive integration deployed (caa1dc1). Attach button now opens a 2-option menu (device / Google Drive); shared `drive-picker.js` module (token fetch, connect prompt w/ auto-resume on tab refocus, lazy gapi); import happens at PICK time via import-to-chat.php (mirrors upload.php storage; records connector_items+imports); chips show importing-spinner/ready-with-Drive-badge/error states; send awaits in-flight imports and skips re-upload. UX browser verification pending (user).
- [x] Executor: Connectors step 7 — Voice Studio "Google Drive" button (next to Upload) → import-drive.php (voice-admin guard, imports into the currently selected folder, mirrors documents/upload.php, records items/imports with context_document_id). Reuses drive-picker.js. Deployed 6805c05. Docs still need the normal Process step for RAG. Browser verification pending.
- [x] Executor: Connectors step 8 — connectors.js: error/needs_attention accounts now show "Reconnect" (→ start.php, callback resets status) with the error message as subtitle. API doc written: docs/apis/google_drive_connector.md. Deployed.
...
- 2026-07-09 Executor: Fixed spurious capability buttons (user report: "any link becomes a voice button"). `extractCapabilityRoutes` (app.php) now (1) only returns routes present in the user's `capabilityRouteIndex` catalog — unknown/hallucinated `/voices|/gestos` paths no longer render generic "Ask Open voice" buttons; (2) skips matches inside absolute URLs of other domains (web-search citations); (3) `enhanceCapabilityActions` defers until the catalog request resolves (idempotent re-run) so early-painted messages keep their buttons. Deployed 189dac7.
- 2026-07-09 Executor: GOOGLE DRIVE CONNECTOR v1 — steps 1-8 COMPLETE & live on prod (commits 7139a9f→5b99ce9). OAuth connect/disconnect/reconnect on /connectors.php; point-of-use pickers in chat composer (attach menu, pick-time import, chip states) and Voice Studio (imports into selected folder, then normal Process step); shared drive-picker.js; docs/apis/google_drive_connector.md. User verified chat flow ("está increíble"); Voice Studio flow user-verified too. Possible future: OneDrive on the same pipeline, re-sync of changed Drive files.
- 2026-06-26 Executor: GLOBAL ACCESS LEVELS — feature FUNCTIONALLY COMPLETE & live on prod (steps 1–5). Engine + admin endpoints verified (smoke 17/17 + 14/14). Voice Studio Access panel = 2-mode switch (Everyone at a level ≥ min / Only specific people) + folder minimums (commit 5a2448b). Users page = global Access levels catalog + per-user level dropdown (commit b308f99). Deploy gotcha hit & documented (root dir not codex-writable → never commit root-file changes; resolved a half-applied tree via setfacl + reset --hard). REMAINING: step 6 only — a human/browser VISUAL REVIEW of both surfaces (no superadmin session available to me), then cleanup of orphaned per-voice endpoints + dropping legacy tables (voice_access_profiles/user_voice_profiles/folder_profile_access). The old `voices/profiles/*` and `voices/access/list|assign` endpoints are now dead (write to ignored tables) but harmless until step 6.
- 2026-06-26 Executor: GLOBAL ACCESS LEVELS — engine cutover DONE & verified on prod (steps 1–3). Migration 026 (additive) + backfill applied after a full backup (`~/claara-backups/iaiapro_db_pre026_*.sql.gz` on the server). New model live: global `access_levels` (seeded "Member" rank 10, default), `users.access_level_id`, `voices.access_mode`('level'|'list') + `min_access_level_id`, `voice_access_list`. Fail-closed cutover: all 3 voices set to `list` mode seeded from `user_voice_profiles` (lex {1,9}; user 11 keeps lex via responsible bypass; test-voice/conveniex superadmin-only). `VoiceAccessResolver` rewritten (reads policy from DB by voice id; list→listed users get all folders, level→rank vs voice min then folder min, NULL=everyone, rank 0 fallback). New `AccessLevelsRepo`/`VoiceAccessListRepo` registered in bootstrap. Smoke test `scripts/smoke_global_access_levels.php` = 17/17 on prod (Part B synthetic level scenario rolled back). Commits 2bac507→c9e7555. **CAVEAT now live:** the old Voice Studio Access panel + `profiles/*` and `access/*` endpoints still write to `user_voice_profiles`, which the new resolver IGNORES — so the admin UI is temporarily disconnected; access can only be changed via SQL until steps 4–5 (endpoints + UI) ship. Remaining: step 4 (global-level CRUD, per-user level, per-voice mode/min/list endpoints), step 5 (UI: where global levels are managed + Access panel rebuild + per-user level), step 6 (drop legacy tables).
