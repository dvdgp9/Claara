...
- [x] T5 Panel en sidebar — IMPLEMENTADO Y VERIFICADO en navegador por el usuario. public/flags.php (gated admin o responsable), enlace "Reports" + badge contador en header-unified.php. Filtros por estado, acciones take/resolve/dismiss/reopen → /api/flags/update.php. Fixes post-verificación: (a) icono iconoir-flag inexistente → iconoir-warning-triangle; (b) sidebar rail sin estilo → reglas .sidebar-rail/.tab-item movidas a styles.css; (c) duplicado asignado/sin-asignar → listAssigned() para admin; (d) mensaje truncado → contenido completo en <details> desplegable.
...
- (decisión de producto, flags sin asignar) Las voces SIN responsable: sus flags caen en la bandeja "Unassigned" que ven SOLO los admins (red de seguridad). Confirmado por el usuario el 2026-06-08. No hay fallback a responsable de departamento.
