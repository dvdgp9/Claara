...
<body class="bg-mesh text-slate-900 overflow-hidden">
  ...
  <div id="share-modal" class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[80vh] flex flex-col">
      ...
      <div class="p-6 border-b border-slate-200 flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center">
            <i class="iconoir-share-android text-xl text-white"></i>
          </div>
          <div>
            <h3 class="text-lg font-semibold text-slate-900">Compartir conversación</h3>
            <p class="text-xs text-slate-500" id="share-conv-title">Gestiona quién tiene acceso</p>
          </div>
        </div>
        <button id="close-share-modal" class="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">
          <i class="iconoir-xmark text-xl"></i>
        </button>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-6">
        ...
        <div class="mb-4">
          <label class="block text-sm font-medium text-slate-700 mb-2">Añadir persona</label>
          <div class="relative">
            <input type="text" id="share-user-search" placeholder="Buscar por nombre o email..." 
                   class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100 transition-colors">
            <i class="iconoir-search absolute right-3 top-1/2 -translate-y-1/2 text-slate-400"></i>
          </div>
          <!-- Resultados de búsqueda -->
          <div id="share-search-results" class="hidden mt-2 bg-white border border-slate-200 rounded-xl shadow-lg max-h-48 overflow-y-auto">
        ...
        <div>
          <div class="text-sm font-medium text-slate-700 mb-2">Personas con acceso</div>
          <div id="share-users-list" class="space-y-2">
            <!-- Se llenará dinámicamente -->
          </div>
          <div id="share-empty-list" class="hidden text-center py-6 text-slate-400 text-sm">
            <i class="iconoir-group text-3xl mb-2"></i>
            <p>Solo tú tienes acceso</p>
          </div>
        </div>
      </div>
      ...
      <!-- Footer -->
      <div class="p-6 border-t border-slate-200 flex gap-3">
        <button id="close-share-btn" class="flex-1 px-4 py-2.5 border-2 border-slate-200 text-slate-700 rounded-lg font-medium hover:bg-slate-50 transition-colors">
          Cerrar
        </button>
      </div>
    </div>
  ...
  <script type="module">
    ...
    let sseConnection = null; // conexión SSE para tiempo real
    let typingDebounceTimer = null; // debounce para reportar escritura
    let isCurrentlyTyping = false; // estado de escritura actual
    ...
    async function loadMessages(conversationId){
      ...
      // Conectar SSE para tiempo real (si la función existe)
