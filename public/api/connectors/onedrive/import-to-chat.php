<?php
/**
 * Imports a OneDrive file (picked via Claara's picker) as a chat attachment,
 * mirroring /api/files/upload.php storage and response shape.
 * POST { item_id: string, conversation_id?: int }
 */
...
try {
    ...
    $fetched = $importer->fetchToTemp($accountId, $itemIdParam, 'chat');
    ...
    error_log('[connectors] OneDrive import-to-chat failed: ' . $e->getMessage());
    if ($itemId) {
        $itemsRepo->updateStatus($itemId, 'error', $e->getMessage());
    }
    if ($importId) {
