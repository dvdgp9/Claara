<?php
/**
 * Google OAuth callback: exchanges the code, stores the account and its
...
if (!empty($_GET['error'])) {
    // User denied the consent screen or Google returned an error.
...
try {
    ...
    if (!in_array('https://www.googleapis.com/auth/drive.file', $grantedScopes, true)) {
        connectors_redirect('error', 'Drive file access was not granted');
    }
    ...
    error_log('[connectors] Google callback failed: ' . $e->getMessage());
    connectors_redirect('error', 'Could not complete the Google connection');
}
