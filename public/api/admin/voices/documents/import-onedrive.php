<?php
/**
 * Imports a OneDrive file (picked via Claara's picker) as a voice knowledge
 * document, mirroring documents/upload.php.
 * POST ?slug=<voice>  Body JSON: { item_id: string, folder_id?: int, description?: string }
 */
...
use Connectors\OneDriveImporter;
...
$itemIdParam = trim((string)($body['item_id'] ?? ''));
if ($itemIdParam === '') {
    Response::error('validation_error', 'item_id is required', 400);
}
...
if (!$account) {
    Response::error('not_connected', 'No connected OneDrive account', 409);
}
...
try {
    $importer = new OneDriveImporter();
    $fetched = $importer->fetchToTemp($accountId, $itemIdParam, 'voice');

    $itemId = $itemsRepo->upsertSelected($accountId, 'onedrive', [
        'external_item_id' => $itemIdParam,
        'item_type' => 'file',
        'name' => $fetched['name'],
        'mime_type' => $fetched['mime'],
        'source_url' => $fetched['metadata']['webUrl'] ?? null,
        'external_version' => $fetched['metadata']['eTag'] ?? null,
    ...
    error_log('[connectors] OneDrive import (voice) failed: ' . $e->getMessage());
    if ($itemId) {
        $itemsRepo->updateStatus($itemId, 'error', $e->getMessage());
    }
    if ($importId) {
    ...
    Response::error('import_failed', 'Could not import the file from OneDrive. Please try again or reconnect your account.', 502);
}
