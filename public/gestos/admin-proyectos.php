...
$headerBackText = I18n::translate('project_ui.all_gestures');
$headerTitle = I18n::translate('project_ui.title');
...
<body class="project-analysis-page bg-mesh text-slate-900 overflow-hidden">
  <div class="min-h-[100dvh] flex h-[100dvh]">
    ...
    <aside id="history-sidebar" class="hidden lg:flex w-72 glass-strong border-r border-slate-200/50 flex-col shrink-0">
      <div class="p-4 border-b border-slate-200/50">
        <div class="flex items-center justify-between">
          <h2 class="font-semibold text-slate-800 flex items-center gap-2">
            ...
            <?= htmlspecialchars(I18n::translate('project_ui.history')) ?>
          </h2>
        </div>
      </div>
      ...
      <div id="history-list" class="flex-1 overflow-auto">
        <div class="p-4 text-center text-slate-400 text-sm">
          ...
          <?= htmlspecialchars(I18n::translate('project_ui.loading')) ?>
        </div>
      </div>
    </aside>
    ...
    $drawerTitle = I18n::translate('project_ui.history');
        ...
        <div class="max-w-3xl mx-auto p-4 lg:p-6 space-y-4 lg:space-y-6">
          ...
          <div class="text-center mb-6">
            <h1 class="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-emerald-600 to-teal-600 mb-2">
              <?= htmlspecialchars(I18n::translate('project_ui.title')) ?>
            </h1>
            <p class="text-slate-500 max-w-lg mx-auto">
              <?= htmlspecialchars(I18n::translate('project_ui.subtitle')) ?>
            </p>
          </div>
            ...
            <form id="project-admin-form" class="space-y-5">
              ...
              <div>
                <label class="block text-sm font-semibold text-slate-700 mb-3">
                  ...
                  <?= htmlspecialchars(I18n::translate('project_ui.documents')) ?>
                </label>
                ...
                <div id="dropzone" class="dropzone flex flex-col items-center justify-center w-full h-32 rounded-xl cursor-pointer hover:border-emerald-400 hover:bg-emerald-50/50 transition-all">
                  ...
                  <span class="text-sm text-slate-500"><?= htmlspecialchars(I18n::translate('project_ui.drop_pdf')) ?></span>
                  <span class="text-xs text-slate-400 mt-1"><?= htmlspecialchars(I18n::translate('project_ui.max_file')) ?></span>
              ...
              <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">
                  ...
                  <?= htmlspecialchars(I18n::translate('project_ui.instructions')) ?> <span class="font-normal text-slate-400"><?= htmlspecialchars(I18n::translate('project_ui.optional')) ?></span>
                </label>
                <textarea id="additional-instructions" rows="3"
                          ...
                          placeholder="<?= htmlspecialchars(I18n::translate('project_ui.instructions_placeholder')) ?>"></textarea>
              </div>
              ...
              <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">
                  ...
                  <?= htmlspecialchars(I18n::translate('project_ui.what_analyze')) ?> <span class="font-normal text-slate-400"><?= htmlspecialchars(I18n::translate('project_ui.select_multiple')) ?></span>
                </label>
                ...
                <div class="grid grid-cols-2 gap-3">
                  <button type="button" data-action="expenses" class="action-card selected p-4 rounded-xl border-2 border-slate-200 text-left">
                    ...
                    <div class="text-sm font-semibold text-slate-800"><?= htmlspecialchars(I18n::translate('project_ui.expenses')) ?></div>
                    <div class="text-xs text-slate-500 mt-1"><?= htmlspecialchars(I18n::translate('project_ui.expenses_help')) ?></div>
                  </button>
                  ...
                  <button type="button" data-action="hours" class="action-card p-4 rounded-xl border-2 border-slate-200 text-left">
                    ...
                    <div class="text-sm font-semibold text-slate-800"><?= htmlspecialchars(I18n::translate('project_ui.hours')) ?></div>
                    <div class="text-xs text-slate-500 mt-1"><?= htmlspecialchars(I18n::translate('project_ui.hours_help')) ?></div>
                  </button>
                </div>
                
                <p id="selected-count" class="text-xs text-emerald-600 font-medium mt-3"><?= htmlspecialchars(I18n::translate('project_ui.selected_one')) ?></p>
              </div>
              ...
              <button type="submit" id="analyze-btn" disabled class="w-full py-3 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white font-semibold rounded-xl shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                ...
                <span id="analyze-btn-text"><?= htmlspecialchars(I18n::translate('project_ui.analyze')) ?></span>
              </button>
              ...
              <div id="progress-panel" class="hidden bg-emerald-50 rounded-xl p-4 border border-emerald-200">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <p id="progress-text" class="text-sm font-medium text-emerald-700"><?= htmlspecialchars(I18n::translate('project_ui.analyzing')) ?></p>
                    <p id="progress-detail" class="text-xs text-emerald-500"><?= htmlspecialchars(I18n::translate('project_ui.extracting')) ?></p>
                  </div>
                </div>
              </div>
              ...
              <div id="error-panel" class="hidden bg-red-50 border border-red-200 rounded-xl p-4">
                <div class="flex items-start gap-2">
                  ...
                  <div>
                    <p class="text-sm font-medium text-red-800"><?= htmlspecialchars(I18n::translate('project_ui.error')) ?></p>
            ...
            <div class="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
              <div class="flex-1 min-w-0">
                <h2 class="text-xl font-bold text-slate-800 flex items-center gap-2 mb-1">
                  ...
                  <span id="result-title" class="truncate"><?= htmlspecialchars(I18n::translate('project_ui.completed')) ?></span>
                </h2>
                <p id="result-source" class="text-sm text-slate-500"><?= htmlspecialchars(I18n::translate('project_ui.tender_analyzed')) ?></p>
              </div>
              <div class="flex items-center gap-3 shrink-0">
                <button type="button" id="copy-result-btn" class="text-sm font-semibold text-slate-600 hover:text-slate-800 flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow transition-all">
                  ...
                  <span><?= htmlspecialchars(I18n::translate('project_ui.copy')) ?></span>
                </button>
                <button type="button" id="new-analysis-btn" class="text-sm font-semibold text-emerald-600 hover:text-white flex items-center gap-2 px-4 py-2 bg-emerald-50 hover:bg-emerald-600 rounded-xl transition-all">
                  ...
                  <span><?= htmlspecialchars(I18n::translate('project_ui.new')) ?></span>
                </button>
              </div>
            </div>
  ...
  <script>
    ...
    window.CLAARA_PROJECT_I18N = <?= I18n::javascriptCatalogPrefixJson('project_ui.') ?>;
  </script>
